...
- B10.1-B10.3 are complete: Rust poller regression covers cold no-media, media starts after app, media app exits, repeated no-media silence, media restart, and session switch between players. Metadata/artwork edge tests cover missing artwork, empty metadata, oversized/unknown artwork bytes, unknown/non-finite duration handling, sparse poller snapshots, and frontend adapter acceptance/rejection. Command capability hardening adds pure frontend gates, `canSkip`-aware controls/shortcuts, and Rust false-command action-context tests. Next work: B10.4 lifecycle edge cases.
