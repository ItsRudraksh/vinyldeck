...
As of this handoff, Backend Phase 0 through Phase 8 are complete and manually approved, including the B8 same-duration track/artwork sync fix. Backend Phase 9 is active: B9.1-B9.5 are implemented and verified. Continue from the next unchecked task in `backend_master_task_list.md`, currently **B9.6**.
...
- B9.1-B9.5 are complete: `src/lib/playback/tauriSource.ts` validates unknown Tauri IPC/event payloads against the Rust `MediaSnapshot` camelCase serde contract, maps null/undefined snapshots to `EMPTY_PLAYBACK`, subscribes to `media-state-changed`, fetches initial state through `cmd_smtc_snapshot`, handles start/stop listener races so retained unlisteners run once, and invokes real `cmd_smtc_*` controls in fire-and-forget mode with bounded error logging. Zustand store owns source teardown through `clearSource(source?)`. Runtime source factory exists at `src/lib/playback/sourceFactory.ts`. Next work: B9.6 wire factory into `src/App.tsx`.
