...
- Mini theme-crossing root cause: main and mini are separate WebViews with separate Zustand memory. Failed attempted fix used `localStorage` as handoff, but each WebView could hold stale values and mini could overwrite the real Tauri Store with default Noir. Final correction: desktop/Tauri settings source is only `tauri-plugin-store`; settings are flushed before opening mini; legacy `vinyldeck:settings-handoff` localStorage is cleared on desktop boot and is not read.
