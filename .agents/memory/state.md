...
- Mini theme-crossing root cause: main and mini are separate WebViews with separate Zustand memory. The correct bridge cannot be raw React state. Current fix writes a synchronous `localStorage` settings handoff before opening mini and keeps that handoff updated during settings saves; `loadSettings()` reads the handoff first, so a freshly-created mini sees the latest theme/customization snapshot immediately instead of waiting for async Store/disk timing.
