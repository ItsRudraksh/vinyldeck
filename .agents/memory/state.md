...
- Mini bug follow-up: user logs showed mini first-create URL is initially `about:blank`, then reused mini loads `http://localhost:1420/`. Mini theme was stale because mode switch opened mini before pending settings persistence flushed. Settings now saves first, then opens native mini. Drag was blocked because full-cover child layers sat above CSS drag region; mini now uses `startDragging()` from background mouse-down with controls/buttons excluded. Closing main after mini return did not exit because hidden mini stayed alive; returning main/fullscreen now destroys the mini window instead of hiding it.
- Mini theme-crossing/settings persistence remains **pending and skipped for now** so backend execution can continue. Confirmed evidence: main and mini are separate WebViews with separate Zustand memory; a failed attempted fix used `localStorage` as handoff, but each WebView could hold stale values and mini could overwrite the real Tauri Store with default Noir. Current code now removes that handoff, uses `tauri-plugin-store` as desktop source of truth, flushes settings before opening mini, and clears legacy `vinyldeck:settings-handoff`; however user wants a second diagnosis before spending more time here. Canonical bug note: `.agents/memory/bugs/BUG-002-mini-theme-persistence.md`.
