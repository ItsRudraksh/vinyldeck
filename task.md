...
- Mini theme crossing/settings persistence bug is **pending and skipped for later**. LocalStorage handoff was wrong for separate WebViews and could poison persistent settings with stale/default Noir. Current code uses Tauri Store only, flushes before mini opens, and clears legacy handoff, but user requested a second-opinion prompt before further fixes. See `.agents/memory/bugs/BUG-002-mini-theme-persistence.md`.
